  ...
  return () => {
    return (tree: Node) => {
      ...
      if (contentsHeadingParent !== null) {
        (contentsHeadingParent as any).children.splice(
          contentsHeadingIndex + 1,
          0,
          {
            type: "html",
            value: contentsHtml,
      ...
      visit(tree, "paragraph", (node: any, index: number, parent: any) => {
        ...
        if (
          textNode.type === "text" &&
          textNode.value.startsWith("::details ")
        ) {
        ...
        if (
          textNode.type === "text" &&
          textNode.value.startsWith("::enddetails")
        ) {
          const node = {
            type: "html",
            value: `</details>`,
        ...
        if (
          textNode.type === "text" &&
          textNode.value.startsWith("::iframe ")
        ) {
      ...
      visit(tree, "link", (node: any) => {
        const isExternal =
          /^https?:\/\//i.test(node.url) || /^\/\//.test(node.url);
